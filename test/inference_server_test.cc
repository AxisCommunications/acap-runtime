...
TEST(Server, ServerAuthentication)
{
  shm_unlink(sharedFile);
...
TEST(Server, ServerInsecureCredentials_Fail)
{
  shm_unlink(sharedFile);
  thread main(ServiceSecurity, 5, cpuChipId, serverCertificatePath, serverKeyPath);
...
TEST(Server, PredictCpuModel1Preload)
{
  shm_unlink(sharedFile);
...
#ifdef __arm__
TEST(Server, ServerAuthenticationTpu)
{
  shm_unlink(sharedFile);
...
TEST(Server, PredictTpuModel1Preload)
{
  shm_unlink(sharedFile);
...
TEST(Server, DISABLED_PredictLoop)
{
  shm_unlink(sharedFile);
